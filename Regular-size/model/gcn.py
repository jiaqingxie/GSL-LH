...
class GCNLayer(Module):
    ...
    def forward(self, input, adj):
        ...
        if input.data.is_sparse:
            ...
            masked_weight = self.weight_mask * self.weight
            support = torch.mm(input, masked_weight)
        output = torch.spmm(adj, support)
        if self.bias is not None:
            masked_bias = self.bias_mask * self.bias
            return output + masked_bias
        else:
            return output
